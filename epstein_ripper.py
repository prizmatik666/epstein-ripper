...
async def stream_download_via_aiohttp(
    ...
    """
    Stream download to part_path, verify PDF signature from first bytes, then rename atomically.
    Returns (ok, err). err may be SESSION_POISON.
    """
    # Reuse Playwright session cookies so DOJ gate/auth stays satisfied
    try:
    ...
    # Keep this generous; unicorn PDFs can be huge
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, headers=headers, allow_redirects=True) as r:
                ...
                # Read just enough to sniff signature without loading whole file
                ...
                # Write streamed response to disk
                with open(part_path, "wb") as f:
                    f.write(first)
                    while True:
                        chunk = await r.content.read(1024 * 1024)  # 1MB chunks
                        if not chunk:
                            break
                        f.write(chunk)
...
async def download_one(context, pdf_url: str, referer: str, out_path: str) -> Tuple[bool, str]:
    ...
    Special behavior:
      ...
      - Avoid Playwright resp.body() for huge PDFs (Playwright protocol string-length cap).
    ...
    # If server provides size and itΓÇÖs large, avoid resp.body() entirely
